...
from splparser.lexers.streamstatslexer import precedence, tokens

BOOLEAN_OPTIONS = ["allnum", "current", "global"]
...
def p_statscmd_cont(p):
    """statscmd : statscmdstart statscmdcont 
                | statscmdstart statscmdcont statsoptlist"""
    p[0] = p[1]
    p[0].add_children(p[2].children)
    correct_groupby(p[0])
    if len(p) > 3:
...
def p_statscmdstart(p):
    """statscmdstart : STREAMSTATS STATS_FN
                     | STREAMSTATS COMMON_FN  
                     | STREAMSTATS EVAL
                     | STREAMSTATS statsoptlist STATS_FN
                     | STREAMSTATS statsoptlist COMMON_FN
                     | STREAMSTATS statsoptlist EVAL"""
...
def p_statsoptlist(p):
    ...
    p[0] = ParseTreeNode('_STREAMSTATS_OPT_LIST')
    p[0].add_child(p[1])
...
def p_statsopt_simplefield(p):
    """statsopt : STREAMSTATS_OPT EQ simplefield"""
...
def p_statscmdstart_statsfnexpr(p):
    """statscmdstart : STREAMSTATS statsfnexpr
                     | STREAMSTATS statsoptlist statsfnexpr"""
    p[0] = ParseTreeNode('COMMAND', raw=p[1])
    fn_idx = 2
    if len(p) > 3:
        fn_idx = 3
...
def p_error(p):
    raise SPLSyntaxError("Syntax error in streamstats parser input!") 
